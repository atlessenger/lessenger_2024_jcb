...
#                                #
# Georgi Marinov                 #
# counts added by Alex Lessenger #
#                                # 
##################################
...
def run():
    ...
    if len(sys.argv) < 5:
        print 'usage: python %s bedfilename chrField BAMfilename chrom.sizes outputfilename [-endonly] [-end2only] [-end1only] [-nomulti] [-RPM] [-counts] [-totalReadNumber number] [-stranded +|-] [-readLength min max] [-printSum] [-uniqueBAM] [-mappabilityNormalize mappability.wig readLength] [-noNH samtools] [-singleFieldRegion] [-fullFragmentMidPoint] [-excludeReadsMappingToOtherChromosomes]' % sys.argv[0]
